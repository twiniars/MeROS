...
\begin{document}
	...
	\title{MeROS: SysML-based Metamodel for ROS-related Systems - version 2.0.0 - reference manual}
...
\label{sec:metamodel-composition}
	...
	The SysML blocks reflect ROS concepts [R3], and their composition is depicted in bdd (block definition diagrams). The metamodel is formulated in a~single SysML package. Hence, Sources Containers (especially Workspaces) and Running Systems are composed into System (Fig.~\ref{fig:ros_system_bdd}). System can aggregate other Systems.
	
		
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=.95]{img/meros_pkg/system_bdd.png}}
		\end{center}
		\caption{System general composition.} 
		\label{fig:ros_system_bdd}
	\end{figure}
	
	Consequently, some concepts (e.g., Node) occur in Workspaces and Running Systems. It reduces the number of SysML blocks in the metamodel [R6.4] and eliminates the need for unnecessary mapping of SysML blocks [R6.5].
	...
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=.95]{img/meros_pkg/communicating_component_specialisations_bdd.png}}
		\end{center}
		\caption{Communicating Component and specialised blocks -- bdd.} 
		\label{fig:communicating_components_bdd}
	\end{figure}
	
	It should be noted that behavioural aspects of a~particular model specified in MeROS can be formulated by operation specification as an act (activity), sd (sequence), or stm (state machine) diagrams. The Running System is one of the aggregates added to the base ROS concepts in MeROS. 
	...
	The Running System (Fig.~\ref{fig:system_bdd}) composes ROS and Non-ROS Communicating Components specializations as well as Connections between them. A~Parameter block is introduced also for ROS~1. In ROS~2, due to safety reasons, Parameter is composed only into Nodes. Data File is introduced as a~general concept to, e.g., store configuration data, store data gathered through experiments or act as a~specific communication medium. Optionally Running System composes the other Running Systems.
	

	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=1.1]{img/meros_pkg/system_bdd.png}}
		\end{center}
		\caption{Running System composition.} 
		\label{fig:system_bdd}
	\end{figure} 
	 	 
	The Communication Channel as as specialisation of the Running System is introduced to specify communication between various Communicating Components. 
	...
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=1.1]{img/meros_pkg/system_communication_bdd.png}}
		\end{center}
	...
	The Namespace (Fig.~\ref{fig:namespace_bdd}) aggregates elements of the Running System, but only ROS related. In opposition to the Running System, the Namespace does not specialise Communicating Component. Hence, it can not act as Communicating Component.
	...
	The Sources Container has various specialisations such as Group of Packages, Workspace, and Versatile Sources Container. It also plays a~role of general container for ROS Packages, while Versatile Sources Container aggregates also Data Files and Non-ROS Communicating Components.  
	The Workspace (Fig.~\ref{fig:sources_container_bdd}) contains of Packages that compose the files related to general ROS concepts such as Node source codes, communication structures definitions, etc. As Workspace has a~specific file-system nature with various types of files stored, Group of Packages were introduced as specific aggregation od ROS Packages only. The Repository, a~specialisation of Versatile Sources Container, plays a~similar role to the Workspace, but reflects the place where sources are stored with version control.
	
	
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=.91]{img/meros_pkg/sources_container_bdd.png}}
		\end{center}
		\caption{Sources Container and its specialisations.} 
		\label{fig:sources_container_bdd}
	\end{figure}
	
	The ROS Package (Fig.~\ref{fig:ros_package_bdd}) composes the files related to general ROS concepts such as Node source codes, communication structures definitions, etc. It should be noted that in case of Actions, specific communication structures definitions are stored in Action Data Structures. The Misc \stblock{} relates to other ROS and Non-ROS files, e.g., roslaunch configuration, obligatory package.xml, CMakeLists.txt. The Metapackage is introduced for conformity with the latest ROS~1 releases [R7] as well as ROS~2. 
	
	
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=.91]{img/meros_pkg/ros_package_bdd.png}}
		\end{center}
...
\label{ch:application-example}
	...
	The Rico \stSystem{} consists of two parts. Its \stWorkspace{} and \stRunSystem{} (Fig.~\ref{fig:rico_system_bdd}).
	
	
	\begin{figure}[H] 
		\centering
		\begin{center}
			{\includegraphics[scale=.85]{img/rico_pkg/rico_system_bdd.png}}
		\end{center}
	...
	Here, the system (\stRunSystem{} \texttt{:Rico}) and its behaviour are formulated in a~general way. An actor (e.g. an elderly person) asks the robot to move. Then, the system recognises the voice command and vocally confirms the command's acceptance. Finally, the robot executes the motion and vocally informs that the motion is finished.
	In the following part of the description, the \stRunSystem{} \texttt{:Rico} and sequence diagram frame \texttt{motion execution} from Fig.~\ref{fig:general_sd} are presented in a~explicit way.
	
	The block definition diagram in Fig.~\ref{fig:rico_system_bdd} depicts the composition of \stRunSystem{} \texttt{:Rico}.
	
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=0.85]{img/rico_pkg/rico_system_bdd.png}}
		\end{center}
		\caption{Rico \stRunSystem{} composition.}
		\label{fig:rico_system_bdd}
	\end{figure}
	
	The \stRunSystem{} \texttt{:Rico} structure is depicted in Fig.~\ref{fig:rico_system_ibd}. Here, and in the following diagrams, the \texttt{rosout} and \texttt{ROS master} \stNode{}s were omitted to make the diagrams more compact. Dedicated block is needed for \stCommChannel{} \texttt{:Move To to Robot Core}, because this \stCommChannel{} is specified in detail later on.
	...
	\begin{figure}[H]
		...
		\end{center}
		\caption{Selected elements of \stRunSystem{} \texttt{:Robot Core}.} 
	...
	The part of the scenario generally described in Fig.~\ref{fig:general_sd} is depicted in detail in Fig.~\ref{fig:motion_execution_sd}. The presentation remains conceptual from the behavioural point of view, but it considers the particular parts of the \stRunSystem{} \texttt{:Rico}.
