...
\begin{document}
	...
	\title{MeROS: SysML-based Metamodel for ROS-related Systems - version 1.2.4 - reference manual}
...
\twci{Dodać plik ze stereotypami w liczbie pojedynczej i mnogiej, podobnie jak w spsysml}
...
\twci{Dodajemy słowo ROS w wymaganiach, tam gdzie jest to potrzebne}
...
\label{ch:requirements}
	...
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=.86]{img/requirement_pkg/structural_aspects_req.png}}
		\end{center}
	...
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=.86]{img/requirement_pkg/original_ros_concepts_req.png}}
		\end{center}
...
\pagebreak
	...
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=0.9]{img/requirement_pkg/final_ros1_release_req.png}}
		\end{center}
...
\label{ch:metamodel}
	...
	The MeROS diagrams were created in the Enterprise Architect development tool within the SysML project [R8] and organised in three packages [R6.1] (Fig.~\ref{fig:meros_project_packages_pkg}): (i) Requirement Model related to requirements formulation and analysis, (ii) MeROS -- the metamodel itself, (iii) Rico Controller -- the exemplary ROS~1 application of MeROS described in sec.~\ref{ch:application-example}. The stereotypes are introduced in MeROS metamodel with the dedicated MeROS profile [R6.2].
	
		
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=1.3]{img/meros_project_packages_pkg.png}}
		\end{center}
	...
	\twci{Dodać i zastosować zasady tworzenia nazw bloków i stereotypów. Nazwa bloku z uwględnieniem ROS, jesli coś jest dla ROS, natomiast w stereotypie jest ROS tylko jeśli ten sam kocenpt jest też w odmianie Non-ROS }
...
\label{sec:metamodel-composition}
	...
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=1.1]{img/meros_pkg/intrasystem_specialisations_bdd.png}}
		\end{center}
		\caption{Instrasystem specialisations -- bdd.} 
	...
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=.9]{img/meros_pkg/ros_package_bdd.png}}
		\end{center}
	...
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=.9]{img/meros_pkg/ros_workspace_bdd.png}}
		\end{center}
...
\label{sec:metamodel-communication}
	...
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=1.1]{img/meros_pkg/topic_communication_with_dedicated_component_ibd.png}}
		\end{center}
...
\label{ch:application-example}
	...
	\begin{figure}[H] 
		\centering
		\begin{center}
			{\includegraphics[scale=1]{img/rico_pkg/general_sd.png}}
		\end{center}
	...
	The <<RunningSystem>> \texttt{:Rico} structure is depicted in Fig.~\ref{fig:rico_running_system_ibd}. Here, and in the following diagrams, the \texttt{rosout} and \texttt{ROS master} <<Node>>s were omitted to make the diagrams more compact. Dedicated block is needed for <<CommChannel>> \texttt{:Move To to Robot Core}, because this <<CommChannel>> is specified in detail later on.
	
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=0.95]{img/rico_pkg/rico_running_system_ibd.png}}
		\end{center}
		\caption{Structure of <<RunningSystem>> \texttt{:Rico} -- ibd.} 
		\label{fig:rico_running_system_ibd}
	\end{figure}

	The system is based on TaskER framework \cite{tasker2020} developed from the RAPP approach to construct systems with variable structure \cite{zielinski2017variable}. The role of the TaskER is to schedule a robot’s tasks. It consists of (i) Task Requesters <<Node>>s to submit new tasks, (ii) Task Harmoniser <<Node>> to schedule tasks execution, (iii) dynamic <<Node>>s (here, <<Node>> \texttt{:Move To}) to execute a particular task on the robot hardware and (iv) cloud part, here <<NRosCompon>> \texttt{:Voice Recognition and Synthesis Platform}. The common part of the controller is located in <<Intrasystem>> \texttt{:Rico}.
	...
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=1.1]{img/rico_pkg/robot_core_ibd.png}}
		\end{center}
	...
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=.85]{img/rico_pkg/motion_execution_sd.png}}
		\end{center}
	...
	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=.85]{img/rico_pkg/command_motion_sd.png}}
		\end{center}
