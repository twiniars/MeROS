...
\label{sec:metamodel-composition}
	...
	The Running System Component (Fig.~\ref{fig:running_system_component_bdd}) is a generalisation of ROS and Non-ROS Communicating Components specializations as well as Connections between them. A~Parameter block is introduced also for ROS~1. In ROS~2, due to safety reasons, Parameter is composed only into Nodes. Database is introduced as a~general concept to, e.g., store configuration data, store data gathered through experiments or act as a~specific communication medium.
	

	\begin{figure}[H]
		\centering
		\begin{center}
			{\includegraphics[scale=1.1]{img/meros_pkg/running_system_component_bdd.png}}
		\end{center}
		\caption{Running System Component specialisations composition.} 
...
\label{ch:application-example}
	...
	The Rico \stSystem{} consists of two parts. Its \stWorkspace{} and \stRunSystemCompon{} (Fig.~\ref{fig:rico_system_bdd}).
	...
	The block definition diagram in Fig.~\ref{fig:rico_system_bdd} depicts the composition of \stSystem{} \texttt{:Rico}.
